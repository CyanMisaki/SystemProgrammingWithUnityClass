...
namespace HW10
{
    ...
    public class StarEditor : Editor
    {
        ...
        private void OnSceneGUI()
        {
            ...
            var angle = -360f / (star.Frequency * star.Points.Points.Length);
            for (var i = 0; i < star.Points.Points.Length; i++)
            {
                var rotation = Quaternion.Euler(0f, 0f, angle * i);
                var oldPoint = starTransform.TransformPoint(rotation * star.Points.Points[i].Position);
                var newPoint = Handles.FreeMoveHandle(oldPoint, Quaternion.identity, 0.02f, Vector3.zero, Handles.DotHandleCap);
                if (oldPoint == newPoint)
                {
                    continue;
                }
                star.Points.Points[i].Position = Quaternion.Inverse(rotation) *
